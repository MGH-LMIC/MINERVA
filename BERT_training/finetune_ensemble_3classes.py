import numpy as np
import torch
from data_loader import DataLoader as MyDataloader
...
class FinetuneBertEnsemble:
    ...
    def test(self, test_dataloader, k=0):
        with torch.no_grad():
            ...
            report = classification_report(y_true=all_targets, y_pred=all_preds, labels=[0, 1, 2],
...
if __name__ == '__main__':
    ...
    if local_model:
        model_name = model_name + '_MLM_v3'
        my_model_to_finetune = 'mask_bert/results/best_bio_bert_v3'
    elif silver_model:
    ...
    folder = 'results_3labels/{}/'.format(model_name)
    ...
    data_loader3 = MyDataloader(data_path=data_path, use_gold=use_gold, split_gold=split_gold, k=2)

    data_loader4 = MyDataloader(data_path=data_path, use_gold=use_gold, split_gold=split_gold, k=3)
    data_loader5 = MyDataloader(data_path=data_path, use_gold=use_gold, split_gold=split_gold, k=4)
    ...
    def change_number_of_labels(dataset):
        new_dataset = {}
        for k, values in dataset.items():
            new_train = copy.deepcopy(values['train'])
            new_train['RELATION'] = new_train['RELATION'].replace(['na'], 'relate')

            new_val = copy.deepcopy(values['validation'])
            new_val['RELATION'] = new_val['RELATION'].replace(['na'], 'relate')

            new_test = copy.deepcopy(values['test'])
            new_test['RELATION'] = new_test['RELATION'].replace(['na'], 'relate')

            new_dataset[k] = {'train': new_train, 'validation': new_val, 'test': new_test}

        return new_dataset

    datasets1 = change_number_of_labels(datasets1)
    datasets2 = change_number_of_labels(datasets2)
    datasets3 = change_number_of_labels(datasets3)
    encode_relations = {'positive': 0, 'negative': 1, 'relate': 2}
    for k in range(5):

        train_dataset1 = MyDataset(datasets1[k]['train'])
        val_dataset1 = MyDataset(datasets1[k]['validation'])
        dicto_classes = list(encode_relations.keys())
        ...
        for i in range(len(train_dataloaders)):
            if local_model:
                ...
                my_model = Model(model, tokenizer, device=device, num_labels=3).to(device)
            elif silver_model:
                tokenizer = AutoTokenizer.from_pretrained(model_to_finetune)
                my_model = torch.load(my_model_to_finetune, num_labels=3)
            else:
            ...
            finetuner = FinetuneBert(model=my_model, model_name=model_name + '_k{}_t{}'.format(k, i), alpha_contrastive=alpha_contrastive,
                                     encode_relations=encode_relations, folder=folder, weights=weights)
        ...
        finetuner_ensemble = FinetuneBertEnsemble(ensemble, model_name=model_name + 'k_{}_ensemble'.format(k),
                                                  encode_relations=encode_relations, folder=folder)
